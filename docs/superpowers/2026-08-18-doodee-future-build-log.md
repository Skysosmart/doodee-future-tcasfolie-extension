...
Ruling 29 — เชื่อมระบบวิเคราะห์พอร์ตของเว็บ (2026-08-25). เว็บมี `POST /api/portfolio/analyse`
อยู่แล้ว รับ `{ text }` แล้วส่งเข้า `analysePortfolioV2` (จัดกลุ่ม -> ดึงคณะที่เป็นไปได้ ->
จัดอันดับ) ไม่ต้องแก้อะไรฝั่งเว็บเลย ของที่ขาดคือฝั่งส่วนขยาย: คลังมีข้อมูลเป็นชิ้น ๆ
ไม่ใช่ข้อความยาว ๆ แบบที่ตัววิเคราะห์กิน

`analysis.js` (pure, 12 เทสต์) ทำสองอย่าง ประกอบข้อความจากคลัง และอ่านคำตอบกลับ
สามอย่างที่ตั้งใจ

- **ตัดที่ 15000 แล้วบอก** เว็บตัดที่ 16000 เงียบ ๆ ถ้าปล่อยไปผู้ใช้จะเชื่อว่าวิเคราะห์ทั้งพอร์ต
  ทั้งที่ส่งไปครึ่งเดียว หน้าจอจึงขึ้นว่า "ส่งไป N จาก M ชิ้น"
- **ตอบ 200 แต่ว่างเปล่าต้องไม่นับว่าสำเร็จ** ถ้าไม่มีทั้ง overview, จุดแข็ง และคณะ ให้โยน error
  ไม่ใช่วาดหน้าเปล่าแล้วบอกว่าเสร็จแล้ว
- **รับได้ทั้ง v2 และ legacy v1** และเผื่อกรณีโมเดลห่อ JSON ด้วย ``` ทั้งที่ system prompt ห้ามไว้

**บั๊กที่เจอตอนทดสอบ: แท็บที่เปิดเองเด้ง** ครั้งแรกได้ "Failed to fetch" ตลอด
แต่พอเปิดแท็บค้างไว้เองแล้วยิง กลับได้ 400 `Text too short` ตามที่ควรเป็น สาเหตุคือ
`sitecall` เปิดแท็บไปที่ `/en/profile/portfolio` ซึ่งเด้งไป `/login` เมื่อยังไม่ล็อกอิน
การนำทางยกเลิก fetch ที่ค้างอยู่ แก้เป็นเปิด `/th` (ไม่เด้งต่อ — `/` เด้งไป `/th` ตาม next-intl)
รอ 600 ms หลัง complete เผื่อ redirect ฝั่ง client และลองซ้ำเมื่อได้ status 0

ครั้งแรกที่ส่วนขยายส่งข้อมูลออกนอกเครื่อง README เดิมประกาศว่า "ไม่มีการยิงข้อมูลออก"
จึงต้องแก้เป็นตารางว่าอะไรออกตอนไหน หน้าวิเคราะห์เตือนก่อนกด และ `site.js` ปฏิเสธ path
ที่ไม่ขึ้นต้นด้วย `/api/` — ยิงออกนอกสองโดเมนที่ประกาศไว้ไม่ได้เลย

Ruling 30 — รับไฟล์ส่งออกโปรไฟล์ของเว็บได้ตรง ๆ (2026-08-27). เจ้าของบอกว่า
`/api/extension/portfolio` ตอบ `items` ว่าง แล้วส่งไฟล์มาให้ดู ไฟล์นั้น**ไม่ใช่คำตอบของ
endpoint นั้น** — คีย์เป็น `exportDate`/`profile`/`statistics` ซึ่งเป็นของ
`/api/profile/export` คนละตัวกัน จึงยังไม่รู้ว่า endpoint ตอบอะไรจริง ๆ

แต่ไฟล์นั้นพิสูจน์ว่า**ข้อมูลมีอยู่จริงและผ่านตัวกรองของ endpoint ทุกข้อ**: ผลงาน 4 ชิ้น
`portfolio_visibility: true`, `verification_status: 'verified'`, `data_source: 'ai_extracted'`
(ผลจาก OCR ของเว็บ) กับกิจกรรมอีก 4 ทุกแถว `user_id` เดียวกัน และ query ของ
`/api/profile/export` กับของ endpoint ต่างกันแค่ `portfolio_visibility: true` ที่เป็น true อยู่แล้ว
ดังนั้น `items: []` อธิบายไม่ได้ด้วยตัวข้อมูล ต้องขอคำตอบดิบมาดูก่อน ไม่เดา

ระหว่างนั้นทำทางที่ใช้ได้ทันที: `siteImport.js` (pure, 14 เทสต์) แปลงไฟล์ส่งออกของเว็บเป็น
รูปที่คลังรับได้ หน้า `backup.html` เดาชนิดไฟล์ให้เอง รับได้ทั้งสองแบบ ไม่ต้องเลือก

จุดที่ต้องระวังและเทสต์คุมไว้: `type`/`level` ต้องเป็นสตริงไทยที่ตรงกับ TCASFolio เป๊ะ ๆ
(เทสต์ยืนยันว่าทุกชิ้นผ่าน `Model.normalize` โดยไม่มี `level` ไหนถูกล้างทิ้ง) ·
เคารพ `portfolio_visibility: false` แล้วรายงานว่าข้ามกี่ชิ้น · `hours_committed: 0`
ต้องไม่กลายเป็นค่าว่าง · คลังไม่มีช่องวันที่ จึงย้าย `date_achieved`/`start_date`-`end_date`
ไปหัว `detail` ไม่ปล่อยให้หาย

ทดสอบกับไฟล์จริงของเจ้าของผ่าน CDP: 4 ผลงาน + 4 กิจกรรม -> เข้าคลัง 8 ชิ้น
หมวดถูกทุกชิ้น (รางวัล 4 · โครงงาน 3 · กิจกรรม 1) ระดับชาติติดครบทั้ง 4 รางวัล
ไฟล์นี้ไม่มีรูป (`certificate_url` เป็น null ทุกแถว) รูปยังต้องมาจาก `ดึงจากเว็บ`
